 ...
 * file located at:
 *
 *   hello-jniCallback/app/src/main/java/com/example/bindroid/MainActivity.java
 */
JNIEXPORT jstring JNICALL
Java_com_example_bindroid_MainActivity_stringFromJNI(JNIEnv *env, jobject thiz,
                                                             jstring inJNIStr) {
#if defined(__arm__)
...
#define ABI "unknown"
    ...
    char *GOOD_PASSWORD = "Well Done ! Use it to validate your Chall !";
    char *WRONG_PASSWORD = "Sorry, i'm unbreakable";
    char *FLAG = "SECURED";

    const char *userInput = (*env)->GetStringUTFChars(env, inJNIStr, NULL);
    (*env)->ReleaseStringUTFChars(env, inJNIStr, userInput);  // release resources

    int length = strlen(userInput);

    if (strcmp(FLAG, userInput) == 0) {
        return (*env)->NewStringUTF(env, GOOD_PASSWORD);
    }
    else {
        return (*env)->NewStringUTF(env, WRONG_PASSWORD);
    }
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    jclass clz = (*env)->FindClass(env,
                                   "com/example/bindroid/JniHandler");
...
Java_com_example_bindroid_MainActivity_startTicks(JNIEnv *env, jobject instance) {
 ...
 */
JNIEXPORT void JNICALL
Java_com_example_bindroid_MainActivity_StopTicks(JNIEnv *env, jobject instance) {
